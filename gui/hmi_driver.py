...
Serialcommand = {
    ...
    # 充电电流
    "setlowcurrent":     ("setlowcurrent",     b""),
    "sethighcurrent":    ("sethighcurrent",    b""),
}
        ...
        if READ_A_RESP == 0:
            ...
            if _serial_key_handle():
                ...
                if DATA_GLO == b"\r\n":  # 空行不作为新包存在
                    ...
                    if DATA_GLO.startswith(b"rtc:"):
                        DATA_GLO = b""
                        ViewMsgASCII("RTC log received.")
                        continue
...
def starthmi(port=PORT_DEFAULT):
    global ser_instance
    ser_instance = DOpenPort(port, 57600, TIMEOUT_S)
...
def ser_byte_mode():
    ...
    DCloseReadThread()
    print("关闭读取线程成功")
    ser_instance = DOpenPort(name, 57600, TIMEOUT_S)
